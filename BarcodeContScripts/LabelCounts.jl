...
fitpath = joinpath(datapath, "GeneFitData", "finalfits")
countpath = joinpath(datapath, "$(cond)_$(rep)_CellCounts.tsv")
outputpath = datapath

# Load parameters set by previous steps
parameter_df = DataFrame(CSV.File(joinpath(fitpath, "$(cond)_$(rep)_Parameters_$(contdist).tsv"), delim='\t'))
...
for BC in names(parameter_df)
    fitparam = parameter_df[!, BC]
    thresh = Integer(fitparam[end])

    gex, gcont, gseq = genfuncs(fitparam[1:end-1]...)
    f = fitparam[end]
    ...
    CSV.write(joinpath(outputpath, "$(cond)_$(rep)_Labeled_$(contdist).tsv"), binarydf, delim='\t')
    CSV.write(joinpath(outputpath, "$(cond)_$(rep)_FalseNeg_$(contdist).tsv"), falsenegdf, delim='\t')
    CSV.write(joinpath(outputpath, "$(cond)_$(rep)_FalsePos_$(contdist).tsv"), falseposdf, delim='\t')
