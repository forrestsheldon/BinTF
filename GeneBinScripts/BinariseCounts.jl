...
for BC in names(parameter_df)
    ν, γ, μ, r, f, thresh = parameter_df[!, BC]
    ...
    pnolist = taylor_expand(gno, order=maxcount).coeffs
    pnoexlist = taylor_expand(gnoex, order=maxcount).coeffs
...
for BC in names(parameter_df)
    ...
    pnolist = taylor_expand(gno, order=maxcountdict[BC]).coeffs
    pnoexlist = taylor_expand(gnoex, order=maxcountdict[BC]).coeffs
