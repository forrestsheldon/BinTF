...
def app(datadir, rep, cond):
    ...
    nofitdir = os.path.join(datadir, "GeneFitData", "cellhistograms")
...
For Genes with low counts in which a fit is not possible, a method of moments run is also available
to obtain an approximate fit. Everything below the threshold is assumed to be noise and everything
equal to or above is assumed to be expression.""")
    ...
    melted_mix_df['Shifted Index'] = melted_mix_df['index'] - 1

    # Create line plot using Plotly Express
    mix_fig = px.line(melted_mix_df, x='Shifted Index', y='Value', color='Column')
    ...
    if st.sidebar.button("Finalise Fit"):
        if new_f == old_f:
            ...
            # Run Tuning Script
            script_path = "./GeneBinScripts/TuneSingleGene.jl"  # Replace with the actual path to your Julia script
    ...
    st.sidebar.markdown("Unfinalised Genes:")
